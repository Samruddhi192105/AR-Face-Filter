import cv2
import mediapipe as mp
import math


# --------------------------------
# MediaPipe Face Landmarker
# --------------------------------

BaseOptions = mp.tasks.BaseOptions
FaceLandmarker = mp.tasks.vision.FaceLandmarker
FaceLandmarkerOptions = mp.tasks.vision.FaceLandmarkerOptions
VisionRunningMode = mp.tasks.vision.RunningMode


options = FaceLandmarkerOptions(
    base_options=BaseOptions(
        model_asset_path="models/face_landmarker.task"
    ),
    running_mode=VisionRunningMode.IMAGE,
    num_faces=1
)


# --------------------------------
# Load sunglasses
# --------------------------------

sunglasses = cv2.imread(
    "assets/sunglasses.png",
    cv2.IMREAD_UNCHANGED
)

if sunglasses is None:
    print("Error: sunglasses.png not found")
    exit()

print("Sunglasses loaded")
print("Sunglasses shape:", sunglasses.shape)


# --------------------------------
# Rotate image while preserving size
# --------------------------------

def rotate_image(image, angle):

    height, width = image.shape[:2]

    center = (
        width // 2,
        height // 2
    )

    rotation_matrix = cv2.getRotationMatrix2D(
        center,
        angle,
        1.0
    )

    rotated = cv2.warpAffine(
        image,
        rotation_matrix,
        (width, height),
        flags=cv2.INTER_LINEAR,
        borderMode=cv2.BORDER_CONSTANT,
        borderValue=(0, 0, 0, 0)
    )

    return rotated


# --------------------------------
# Create Face Landmarker
# --------------------------------

with FaceLandmarker.create_from_options(options) as landmarker:

    cap = cv2.VideoCapture(0)

    if not cap.isOpened():
        print("Error: Could not open webcam")
        exit()

    print("AR sunglasses started")
    print("Press Q to quit")

    while True:

        # --------------------------------
        # Read frame
        # --------------------------------

        success, frame = cap.read()

        if not success:
            print("Failed to read frame")
            break

        # Mirror webcam
        frame = cv2.flip(frame, 1)

        height, width, _ = frame.shape

        # --------------------------------
        # BGR → RGB
        # --------------------------------

        rgb_frame = cv2.cvtColor(
            frame,
            cv2.COLOR_BGR2RGB
        )

        mp_image = mp.Image(
            image_format=mp.ImageFormat.SRGB,
            data=rgb_frame
        )

        # --------------------------------
        # Detect landmarks
        # --------------------------------

        result = landmarker.detect(mp_image)

        if result.face_landmarks:

            face_landmarks = result.face_landmarks[0]

            # --------------------------------
            # Eye landmarks
            # --------------------------------

            LEFT_EYE_1 = 33
            LEFT_EYE_2 = 133

            RIGHT_EYE_1 = 362
            RIGHT_EYE_2 = 263

            left_1 = face_landmarks[LEFT_EYE_1]
            left_2 = face_landmarks[LEFT_EYE_2]

            right_1 = face_landmarks[RIGHT_EYE_1]
            right_2 = face_landmarks[RIGHT_EYE_2]

            # --------------------------------
            # Convert normalized → pixels
            # --------------------------------

            left_p1 = (
                int(left_1.x * width),
                int(left_1.y * height)
            )

            left_p2 = (
                int(left_2.x * width),
                int(left_2.y * height)
            )

            right_p1 = (
                int(right_1.x * width),
                int(right_1.y * height)
            )

            right_p2 = (
                int(right_2.x * width),
                int(right_2.y * height)
            )

            # --------------------------------
            # Eye centers
            # --------------------------------

            left_center = (
                (left_p1[0] + left_p2[0]) // 2,
                (left_p1[1] + left_p2[1]) // 2
            )

            right_center = (
                (right_p1[0] + right_p2[0]) // 2,
                (right_p1[1] + right_p2[1]) // 2
            )

            # --------------------------------
            # Eye distance
            # --------------------------------

            dx = right_center[0] - left_center[0]
            dy = right_center[1] - left_center[1]

            eye_distance = math.sqrt(
                dx ** 2 + dy ** 2
            )

            # --------------------------------
            # Calculate head tilt angle
            # --------------------------------

            angle = -math.degrees(
                math.atan2(dy, dx)
            )

            # --------------------------------
            # Resize sunglasses
            # --------------------------------

            sunglasses_width = int(
                eye_distance * 2.7
            )

            sunglasses_height = int(
                sunglasses.shape[0]
                * sunglasses_width
                / sunglasses.shape[1]
            )

            resized = cv2.resize(
                sunglasses,
                (
                    sunglasses_width,
                    sunglasses_height
                ),
                interpolation=cv2.INTER_AREA
            )

            # --------------------------------
            # Rotate sunglasses
            # --------------------------------

            rotated = rotate_image(
                resized,
                angle
            )

            rotated_height, rotated_width = rotated.shape[:2]

            # --------------------------------
            # Eye center
            # --------------------------------

            center_x = (
                left_center[0] +
                right_center[0]
            ) // 2

            center_y = (
                left_center[1] +
                right_center[1]
            ) // 2
            
            # --------------------------------
            # Position sunglasses
            # --------------------------------

            x1 = int(
                center_x -
                rotated_width / 2
            )

            y1 = int(
                center_y -
                rotated_height / 2
            )

            x2 = x1 + rotated_width
            y2 = y1 + rotated_height

            # --------------------------------
            # Check boundaries
            # --------------------------------

            if (
                x1 >= 0 and
                y1 >= 0 and
                x2 <= width and
                y2 <= height
            ):

                # --------------------------------
                # Alpha blending
                # --------------------------------

                if rotated.shape[2] == 4:

                    alpha = (
                        rotated[:, :, 3]
                        / 255.0
                    )

                    for c in range(3):

                        frame[
                            y1:y2,
                            x1:x2,
                            c
                        ] = (
                            alpha *
                            rotated[:, :, c]
                            +
                            (1 - alpha) *
                            frame[
                                y1:y2,
                                x1:x2,
                                c
                            ]
                        )

                else:

                    frame[
                        y1:y2,
                        x1:x2
                    ] = rotated

            # --------------------------------
            # Display angle
            # --------------------------------

            cv2.putText(
                frame,
                f"Angle: {angle:.2f}",
                (20, 40),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.7,
                (255, 255, 255),
                2
            )

            cv2.putText(
                frame,
                f"Eye Distance: {eye_distance:.2f}",
                (20, 70),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.7,
                (255, 255, 255),
                2
            )

        # --------------------------------
        # Display
        # --------------------------------

        cv2.imshow(
            "AR Sunglasses",
            frame
        )

        if cv2.waitKey(1) & 0xFF == ord("q"):
            break

    cap.release()
    cv2.destroyAllWindows()